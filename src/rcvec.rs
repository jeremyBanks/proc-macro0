...
impl<T> RcVec<T> {
    ...
    {
        RcVecMut {
            inner: SmartPtr::make_mut(&mut self.inner),
        }
    }

    pub(crate) fn get_mut(&mut self) -> Option<RcVecMut<T>> {
        let inner = SmartPtr::get_mut(&mut self.inner)?;
        Some(RcVecMut { inner })
    }
    ...
    where
        T: Clone,
    {
        let vec = if let Some(owned) = SmartPtr::get_mut(&mut self.inner) {
            mem::take(owned)
        } else {
...
impl<T> Clone for RcVec<T> {
    fn clone(&self) -> Self {
        RcVec {
            inner: SmartPtr::clone(&self.inner),
        }
    }
}
...
// When sync feature is enabled, RcVec is Send + Sync
#[cfg(feature = "sync")]
unsafe impl<T: Send + Sync> Send for RcVec<T> {}
#[cfg(feature = "sync")]
unsafe impl<T: Send + Sync> Sync for RcVec<T> {}
